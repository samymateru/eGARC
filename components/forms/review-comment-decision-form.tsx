...
type ReviewCommentDecisionValues = z.infer<typeof ReviewCommentDecisionSchema>;

interface ReviewCommentDecisionFormProps {
...
export const ReviewCommentDecisionForm = ({
  children,
  id,
  endpoint,
  title,
}: ReviewCommentDecisionFormProps) => {
  const [open, setOpen] = useState(false);
  const params = useSearchParams();
  const methods = useForm<ReviewCommentDecisionValues>({
    resolver: zodResolver(ReviewCommentDecisionSchema),
  });

  const query_client = useQueryClient();

  const {
    mutate: reviewCommentDecision,
    isPending: reviewCommentDecisionPending,
  } = useMutation({
    mutationKey: ["_review_comment_decision"],
    mutationFn: async (
      data: ReviewCommentDecisionValues
    ): Promise<Response> => {
  ...
  const onSubmit = (data: ReviewCommentDecisionValues) => {
    reviewCommentDecision(data, {
      onSuccess: (data) => {
        ...
        <AlertDialogContent className="p-0 max-w-[500px] dark:bg-black">
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <footer className="flex justify-center gap-2 p-4">
              ...
              <Button
                disabled={reviewCommentDecisionPending}
                type="submit"
                variant="ghost"
